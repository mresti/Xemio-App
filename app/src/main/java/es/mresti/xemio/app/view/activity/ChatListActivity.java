...
import es.mresti.xemio.app.presenter.ChatListPresenter;
...
public class ChatListActivity extends BaseActivity implements ChatListContract.View {
  ...
  private ChatListContract.UserActionsListener mActionsListener;
  ...
  private void initialize() {
    ...
    mActionsListener = new ChatListPresenter(this);
    setSupportActionBar(mToolbar);
    if (getSupportActionBar() != null) {
      getSupportActionBar().setDisplayHomeAsUpEnabled(true);
    }
    mChannelsDatasRef = mActionsListener.getChatListRef();
        ...
        new FirebaseListAdapter<HashMap>(mChannelsDatasRef, HashMap.class, R.layout.list_1_item,
          ...
          @Override protected void populateView(View v, final HashMap model) {
            ((TextView) v.findViewById(R.id.item_title)).setText(
                model.get("foundation").toString());
            v.setClickable(true);
            v.setOnClickListener(new View.OnClickListener() {
              @Override public void onClick(View v) {
                selectChannelItem(model.get("chatId").toString());
              }
            });
          }
        };
  ...
  private void selectChannelItem(String key) {
    Intent intent = new Intent(this.getContext(), ChatMessageActivity.class);
